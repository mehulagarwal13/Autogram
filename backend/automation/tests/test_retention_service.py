...
#
# There is no per-application generated résumé/cover-letter in this
# codebase — `Application.resume_used` is a plain FK into the user's own
# PERMANENT `ProfileDocument` library, and purging one just because one
...
# cleanup). `RetentionPolicy` deliberately has no `document_retention_days`
# column for exactly this reason (see its docstring). This test locks in
# the real invariant that matters regardless of that column's existence:
